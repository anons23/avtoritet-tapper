...
(function(){
  ...
  const DEALS=[
    ...
    {title:'Пропавшая посылка',story:'Посылка пришла без подписи. Несколько человек уверяют, что она их. Нужно установить порядок.',img:'hero',choices:[['Проверить список',200,35,'safe'],['Разделить содержимое',230,40,'safe'],['Выслушать всех',190,45,'clash']]},
    {title:'Шум в бараке',story:'Двое спорят из-за шума после отбоя. Спор быстро собирает зрителей.',img:'clash',choices:[['Развести по местам',210,40,'safe'],['Поговорить один на один',190,35,'clash'],['Поставить точку',270,45,'clash']]},
    {title:'Долг за услугу',story:'Один человек требует вернуть долг, второй говорит, что срок ещё не вышел. Оба пришли к тебе.',img:'hero',choices:[['Дать срок',160,35,'safe'],['Разделить долг',230,40,'safe'],['Назначить решение',280,45,'clash']]},
    {title:'Кому достанется место',story:'Освободилась койка в более тихом углу барака. За неё уже спорят двое.',img:'clash',choices:[['Жребий',190,40,'safe'],['По очереди',180,35,'safe'],['Решить по ситуации',250,45,'clash']]}
  ];
  ...
  function closeAuthorityModal(){stopClash();const overlay=$('modal-overlay'),modal=$('modal');if(overlay)overlay.classList.add('hidden');if(overlay)overlay.dataset.locked='0';if(modal)modal.classList.remove('authority-modal');modalOpen=false;dealResolving=false}
  function reward(influence,respect,bonus){const s=state();if(!s)return;s.points=(Number(s.points)||0)+influence;s.chifir=(Number(s.chifir)||0)+Math.max(0,bonus||0);s.respect=(Number(s.respect)||0)+Math.max(0,respect||0);s.tasks=s.tasks||{};s.tasks.earned=(Number(s.tasks.earned)||0)+Math.max(0,bonus||0);s.tasks.authorityDeals=(Number(s.tasks.authorityDeals)||0)+1;try{if(typeof window.checkTasks==='function')window.checkTasks();if(typeof window.saveGame==='function')window.saveGame()}catch(e){}const p=$('points'),r=$('respect-stat'),c=$('chifir');if(p)p.textContent=fmt(s.points);if(r)r.textContent=fmt(s.respect);if(c)c.textContent=fmt(s.chifir)}
  ...
  function spendEnergy(amount){const s=state();amount=Math.max(0,Math.floor(amount||0));if(!s)return false;if(typeof s.energy!=='number')s.energy=0;if(s.energy<amount){setMessage('⚡ Не хватает энергии. Нужно '+amount+'.');return false}s.energy-=amount;if(typeof s.lastEnergyTime!=='number')s.lastEnergyTime=Date.now();s.saveUpdatedAt=Date.now();const e=$('energy');if(e)e.textContent=Math.floor(s.energy);try{if(typeof window.saveGame==='function')window.saveGame()}catch(err){}return true}
  ...
  function startDeal(free){if(modalOpen)return;dealResolving=false;const s=state();if(!s||Number(s.points)<40000||s.currentObject!==4||s.jailed)return;if(!free&&!spendEnergy(3))return;const d=DEALS[Math.floor(Math.random()*DEALS.length)];const support=Math.random()<.28?SUPPORTS[Math.floor(Math.random()*SUPPORTS.length)]:null;openAuthorityModal('<div class="authority-deal-window"><div class="authority-modal-head"><span class="authority-modal-icon">📋</span><div><small>ДЕЛО БАРАКА</small><h2>'+esc(d.title)+'</h2></div></div><div class="authority-deal-art">'+img(d.img,'authority-deal-art-img')+'</div><p class="authority-story">'+esc(d.story)+'</p>'+(support?'<div class="authority-support">🤝 <b>'+esc(support[0])+':</b> '+esc(support[1])+' <strong>+'+support[2]+' влияние</strong></div>':'')+'<div class="authority-choice-title">КАК ПОСТУПИТЬ?</div><div class="authority-choices">'+d.choices.map((c,i)=>'<button type="button" class="authority-choice" data-choice="'+i+'"><span>'+['💬','⚖️','👑'][i]+'</span><b>'+esc(c[0])+'</b><small>+'+c[1]+' влияние</small></button>').join('')+'</div></div>',true);document.querySelectorAll('.authority-choice').forEach(b=>b.addEventListener('click',()=>resolveDeal(d,Number(b.dataset.choice),support)))}
  function resolveDeal(d,index,support){if(!modalOpen||dealResolving)return;dealResolving=true;const c=d.choices[index];if(!c){dealResolving=false;return;}const success=Math.random()>=c[2]/100;if(support&&success)reward(support[2],1,0);if(success&&(c[3]==='safe'||Math.random()<.35)){reward(c[1],1,Math.floor(c[1]/8));showResult(true,c[1]+(support?support[2]:0),1,'Конфликт улажен. В бараке снова порядок.');return}reward(Math.floor(c[1]*.2),0,0);startClash({title:d.title,base:c[1],respect:2+(index===2?1:0),image:Math.random()<.2?'fight':'clash'})}
  function startClash(info){stopClash();const target=12+Math.floor(Math.random()*5),time=4200;clashState={...info,target,player:0,opponent:0,started:performance.now(),time};openAuthorityModal('<div class="authority-clash-window"><div class="authority-modal-head"><span class="authority-modal-icon">⚡</span><div><small>СТЫЧКА</small><h2>Кто быстрее?</h2></div></div><div class="authority-clash-art">'+img(info.image,'authority-clash-img')+'<div class="authority-art-vignette"></div></div><p class="authority-story">Спор перешёл в короткую проверку реакции. Нажимай быстро и набери <b>'+target+'</b> очков раньше оппонента.</p><div class="clash-score-row"><div><small>ТВОЁ ВЛИЯНИЕ</small><b id="authority-player-score">0 / '+target+'</b></div><div><small>ОППОНЕНТ</small><b id="authority-opponent-score">0 / '+target+'</b></div></div><div class="clash-bars"><div class="clash-bar"><span id="authority-player-bar"></span></div><div class="clash-bar opponent"><span id="authority-opponent-bar"></span></div></div><button type="button" class="authority-pressure-btn" id="authority-pressure">'+img('pressure','authority-pressure-img')+'</button><div class="authority-clash-timer" id="authority-clash-timer">4.2</div><div class="authority-clash-note">Нажимай по кнопке как можно быстрее</div></div>',true);oppTimer=setInterval(()=>{if(!clashState)return;clashState.opponent+=1+(Math.random()<.2?1:0);updateClash();checkClashEnd()},330);clashTimer=setInterval(()=>{if(!clashState)return;const left=Math.max(0,clashState.time-(performance.now()-clashState.started));const el=$('authority-clash-timer');if(el)el.textContent=(left/1000).toFixed(1);if(left<=0)finishClash(false)},50)}
