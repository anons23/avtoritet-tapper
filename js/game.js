...
let s={chifir:0,points:0,energy:250,maxEnergy:250,power:1,critChance:.05,respect:0,wealth:0,nickname:'',currentObject:0,prestige:0,upgrades:{power:0,crit:0,energyMax:0},boosters:{double:0},tasks:{taps:0,crit:0,events:0,earned:0,jail:0,bugorSuccess:0,npcSuccess:0},completed:{},achievements:{},lastEnergyTime:Date.now(),saveUpdatedAt:Date.now(),lastChoiceEvent:0,totalTaps:0,jailed:false,jailTaps:0,jailRequired:500,confiscatedChifir:0,jailProtection:0,authorityTaps:0,energyRepairVersion:0};
...
function load(){try{const raw=localStorage.getItem('avtoritet_save_v2');if(raw){const d=JSON.parse(raw);if(d&&typeof d==='object')s={...s,...d,upgrades:{...s.upgrades,...(d.upgrades||{})},tasks:{...s.tasks,...(d.tasks||{})},boosters:{...s.boosters,...(d.boosters||{})},achievements:{...s.achievements,...(d.achievements||{})}}}}catch(e){}if(!s.nickname)s.nickname=N[Math.floor(Math.random()*N.length)];if(!Number.isFinite(s.energy))s.energy=250;if(Number(s.energy)<=0&&!Number(s.energyRepairVersion)){s.energy=Math.max(1,Number(s.maxEnergy)||250);s.lastEnergyTime=Date.now();s.energyRepairVersion=1;}if(!Number.isFinite(s.maxEnergy))s.maxEnergy=250;if(!Number.isFinite(s.lastEnergyTime))s.lastEnergyTime=Date.now();if(!Number.isFinite(s.saveUpdatedAt))s.saveUpdatedAt=Date.now();if(!Number.isFinite(s.jailTaps)||s.jailTaps<0)s.jailTaps=0;if(!Number.isFinite(s.jailRequired)||s.jailRequired<1)s.jailRequired=500;if(!Number.isFinite(s.confiscatedChifir)||s.confiscatedChifir<0)s.confiscatedChifir=0;if(!Number.isFinite(s.jailProtection)||s.jailProtection<0)s.jailProtection=0;if(!Number.isFinite(s.tasks.bugorSuccess))s.tasks.bugorSuccess=0;if(!Number.isFinite(s.tasks.npcSuccess))s.tasks.npcSuccess=0;restoreEnergy(Date.now());syncObject(false);tasksCheck();save()}
