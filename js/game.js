...
let s={cigarettes:0,points:0,energy:250,maxEnergy:250,power:1,critChance:.05,respect:0,wealth:0,nickname:'',currentObject:0,prestige:0,upgrades:{power:0,crit:0,energyMax:0},boosters:{double:0},tasks:{taps:0,crit:0,events:0,earned:0,jail:0,bugorSuccess:0,npcSuccess:0},completed:{},achievements:{},lastEnergyTime:Date.now(),saveUpdatedAt:Date.now(),lastChoiceEvent:0,totalTaps:0,jailed:false,jailTaps:0,jailRequired:500,confiscatedCigarettes:0,jailProtection:0,authorityTaps:0,energyRepairVersion:0};
...
function load(){try{const raw=localStorage.getItem('avtoritet_save_v2');if(raw){const d=JSON.parse(raw);if(d&&typeof d==='object')s={...s,...d,upgrades:{...s.upgrades,...(d.upgrades||{})},tasks:{...s.tasks,...(d.tasks||{})},boosters:{...s.boosters,...(d.boosters||{})},achievements:{...s.achievements,...(d.achievements||{})}}}}catch(e){}if(!s.nickname)s.nickname=N[Math.floor(Math.random()*N.length)];if(!Number.isFinite(s.energy))s.energy=250;if(Number(s.energy)<=0&&!Number(s.energyRepairVersion)){s.energy=Math.max(1,Number(s.maxEnergy)||250);s.lastEnergyTime=Date.now();s.energyRepairVersion=1;}if(!Number.isFinite(s.maxEnergy))s.maxEnergy=250;if(!Number.isFinite(s.lastEnergyTime))s.lastEnergyTime=Date.now();if(!Number.isFinite(s.saveUpdatedAt))s.saveUpdatedAt=Date.now();if(!Number.isFinite(s.jailTaps)||s.jailTaps<0)s.jailTaps=0;if(!Number.isFinite(s.jailRequired)||s.jailRequired<1)s.jailRequired=500;if(!Number.isFinite(s.confiscatedCigarettes)||s.confiscatedCigarettes<0)s.confiscatedCigarettes=0;if(!Number.isFinite(s.jailProtection)||s.jailProtection<0)s.jailProtection=0;if(!Number.isFinite(s.tasks.bugorSuccess))s.tasks.bugorSuccess=0;if(!Number.isFinite(s.tasks.npcSuccess))s.tasks.npcSuccess=0;restoreEnergy(Date.now());syncObject(false);tasksCheck();save()}
...
function releaseFromJail(){const confiscated=Math.max(0,Math.floor(s.confiscatedCigarettes));s.jailed=false;s.confiscatedCigarettes=0;s.jailTaps=0;s.tasks.jail=(s.tasks.jail||0)+1;s.jailProtection=75;msg('🔓 Карцер пройден! Тебя выпустили. 🚬 Изъято чефира: '+fmt(confiscated));tasksCheck();ui();save()}
function enterJail(reason){if(s.jailed)return;s.jailed=true;s.jailRequired=500;s.jailTaps=0;const jailRate=.20+Math.random()*.05;s.confiscatedCigarettes=Math.min(Math.max(0,Math.floor(s.cigarettes)),Math.floor(Math.max(0,s.cigarettes)*jailRate));s.cigarettes=Math.max(0,s.cigarettes-s.confiscatedCigarettes);s.jailProtection=0;activeEvent=false;const o=$('modal-overlay');if(o){o.dataset.locked='0';o.classList.add('hidden')}msg('🚨 Ты загремел в карцер из-за своего буйного характера. Надзиратели нашли твой тайник и забрали '+fmt(s.confiscatedCigarettes)+' 🚬 чефира.');if(reason)msg(reason+' Отсидеть: 500 тапов.');ui();save()}
...
function more(){if(s.jailed)return;const ach=Object.entries(ACHIEVEMENTS).map(([id,a])=>{const done=!!s.achievements[id];return '<div class="achievement-card '+(done?'achievement-open':'achievement-locked')+'"><div class="achievement-icon">'+(done?a.icon:'🔒')+'</div><div><b>'+(done?a.title:'Скрытое достижение')+'</b><p>'+(done?a.desc:'Разблокируется только после выполнения условия.')+'</p></div></div>'}).join('');openModal('<div class="section-window barrack-window"><div class="section-kicker">ТВОЁ МЕСТО</div><h2>☰ Барак</h2><p class="section-subtitle">Здесь можно поговорить с персонажей и открыть достижения. Не выполненные достижения остаются скрытыми.</p><div class="barrack-achievements"><div class="section-mini-title">🏆 Достижения</div>'+ach+'</div><div class="prestige-card"><div><b>💎 Новый срок</b><p>После высшей масти можно начать новый срок, сохранив часть бонусов.</p></div><button type="button" class="prestige-btn" disabled>Скоро</button></div></div>')}
function bind(){load();ui();$('tap-area')?.addEventListener('pointerdown',tap,{passive:false});$('btn-shop')?.addEventListener('click',shop);$('btn-rank')?.addEventListener('click',rankMenu);$('btn-tasks')?.addEventListener('click',tasksMenu);$('btn-more')?.addEventListener('click',more);$('modal-close')?.addEventListener('click',closeModal);setInterval(()=>{restoreEnergy(Date.now());ui();save()},1000)}
