...
(function(){
  ...
  function openNameEditor(){
    ...
    function applyName(){
      ...
      if(!value)return;
      try{const state=typeof window.getGameState==='function'?window.getGameState():null;if(!state)return;state.nickname=value.slice(0,24);if(typeof window.saveGame==='function')window.saveGame();if($('nickname'))$('nickname').textContent=state.nickname;modal.classList.add('hidden')}catch(e){}
    }
  }
  ...
  function init(){
    ...
    try{const state=typeof window.getGameState==='function'?window.getGameState():null;if(state&&state.nickname&&OLD_DEFAULTS.includes(state.nickname)){state.nickname=randomName();if(typeof window.saveGame==='function')window.saveGame();if(nick)nick.textContent=state.nickname}}catch(e){}
    hideOldNameBlock();
