...
(function(){
  ...
  function card(id,n){
    const st=state(id),r=getRep(id),lv=level(id),chance=successChance(id),unavailable=!unlocked(id);let note='';
    if(unavailable)note='🔒 Откроется с мастью «'+RANKS[n.rank]+'»';else if(st.locked)note='⏳ Отдохни ещё '+cooldownText(st.until);else if(st.total<=0)note='🎬 Ещё 2 обращения можно получить за рекламу';else note='Осталось обращений: '+st.total;
  ...
  function openNpc(id){
    ...
    setTimeout(()=>{const c=$('modal-content');if(!c)return;c.innerHTML='<h2>'+n.icon+' '+n.name+'</h2>'+card(id,n)+'<button type="button" id="npc-back" style="width:100%;margin-top:6px">Вернуться к бараку</button>';const b=c.querySelector('[data-custom-npc]');if(b)b.onclick=()=>useNpc(id);const back=$('npc-back');if(back)back.onclick=render},20);
  }
  ...
  function useNpc(id){
    const n=NPCS[id];if(!n||!unlocked(id))return;const st=state(id);if(st.locked){msgLocal('⏳ '+n.name+' отшил тебя. Ещё '+cooldownText(st.until)+'.');return}if(st.total<=0){offerAd(id);return}
    const d=data(id);if(d.used<MAX_FREE)d.used++;else if(d.extra>0)d.extra--;if(d.used>=MAX_FREE&&d.extra<=0)d.until=Date.now()+COOLDOWN;uses[id]=d;save();
    const oldMsg=window.msg;let result='';if(typeof oldMsg==='function')window.msg=x=>{result=String(x)};
    try{window.npcMenu();const real=document.querySelector('[data-npc="'+id+'"]');if(real)real.click()}finally{if(typeof oldMsg==='function')window.msg=oldMsg}
