...
(function(){
  ...
  function msgLocal(t){if(typeof window.msg==='function'){window.msg(t);return}const e=$('event-message');if(!e)return;e.textContent=t;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),3200)}
  ...
  function useNpc(id){if(typeof window.npcMenu==='function'){}if(totalUses()<1){offerAd();return}if(typeof window.msg!=='function'||typeof window.NPCS_RUN!=='function'){msgLocal('⚠️ NPC временно недоступны.');return}if(!consume())return;save();try{window.NPCS_RUN(id)}catch(e){state.free++;save();msgLocal('⚠️ Не удалось выполнить действие. Использование возвращено.')}render()}
  function menu(){if(typeof window.npcMenu==='function'){window.npcMenu();setTimeout(()=>{document.querySelectorAll('[data-npc]').forEach(b=>{if(b.dataset.npcBound)return;b.dataset.npcBound='1';b.addEventListener('click',()=>useNpc(b.dataset.npc))});render()},0)}}
