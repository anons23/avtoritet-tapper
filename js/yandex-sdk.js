...
(function(){
  ...
  function queueCloudSave(value, flush){
    ...
    if(flush){ flushCloudSave(); return; }
    saveTimer=setTimeout(flushCloudSave,2000);
  }
  ...
  async function init(){
    if(!window.YaGames){ log('SDK loader is unavailable; local save remains active.'); return; }
    try{
      ...
      if(ysdk.on){
        ysdk.on('game_api_pause',function(){ flushCloudSave(); });
  ...
  window.showRewardedAd=function(onReward){
    ...
    try{
      ysdk.adv.showRewardedVideo({
        callbacks:{
          onRewarded:function(){ rewarded=true; if(typeof onReward==='function') onReward(true); },
  ...
  window.YandexGameBridge={
    getSDK:function(){return ysdk;},
